...
# The file should be in the zip file downloaded from the described lines 40-44.
...
# Examine the warning messages and try to resolve it.
...
original_austen %>%
  ...
  comparison.cloud(colors = c("gray20", "gray80"), max.words = 100,
                   fixed.asp=TRUE)
# Plot looks different. Save the plot to add back the header/footer
...
original_austen %>% # x = original_austen, y = bing_negative
  ...
  top_n(1) %>% # we only want one chapter (the most 'negative' one) from each book
  ungroup()